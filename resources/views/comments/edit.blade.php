...
@section('content')
    ...
    {{-- <form action="{{ route('posts.comments.update', [$post, $comment]) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" id="title" class="form-control" value="{{ $comment->title }}">
        </div>

        <div class="form-group">
            <label for="description">Description:</label>
            <textarea name="description" id="description" class="form-control">{{ $comment->description }}</textarea>
        </div>

        <button type="submit" class="btn btn-primary">Update</button>
    </form> --}}

    <form action="{{ route('comments.update', $comment) }}" method="POST" class="w-1/2">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div class="border-b border-gray-900/10 pb-12">
                ...
                <p class="mt-1 text-sm leading-6 text-gray-600">You can edit comment</p>
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
            <button type="button" class="btn-secondary"
                onclick="window.location='{{ route('posts.show', $comment->post) }}'">Cancel</button>
    ...
    <form action="{{ route('comments.destroy', $comment) }}" method="POST" class="d-inline">
        ...
        <button type="submit" onclick="return confirm('{{ __('Are you sure you want to delete?') }}')"
            class="btn-danger">Delete Comment</button>
    </form>
@endsection
